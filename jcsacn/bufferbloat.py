...
import os
import math
import numpy as num ##imported numpy to find standard deviation , mean , web download time etc.
...
def start_iperf(net):
    ...
    h1.popen("iperf -t %s -c %s > %s/iperf.txt" % (time, h2.IP(),args.dir),shell=True)
...
def bufferbloat():
    ...
    wdt = num.array(web_download_time).astype(num.float)
    # TODO: Move to readme
   
    f = open('./avg_web.txt', 'w+')
